  ...
  <div class="bg-[var(--t-bg)] border border-[var(--t-border)] rounded-xl p-3 space-y-3">
      ...
      <div class="flex-1 min-w-0 space-y-2">
        ...
        <div class="flex flex-wrap items-center gap-2.5">
          <span class="px-3 py-1 rounded-full text-[0.75rem] font-semibold uppercase tracking-wider tabular-nums"
            style="background:color-mix(in srgb, #2563eb 12%, transparent);color:var(--t-ink)">
            RPC: {record.registration_number}
          </span>
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[0.75rem] font-semibold uppercase tracking-wider"
            style="background:color-mix(in srgb, {categoryColor(record.category)} 14%, transparent);color:var(--t-ink)">
            <span class="h-2 w-2 rounded-full shrink-0" style="background:{categoryColor(record.category)}"></span>
            {record.category}
          </span>
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[0.75rem] font-semibold uppercase tracking-wider"
            style="background:{record.status === 'Active' ? 'rgba(0,204,102,0.14)' : 'rgba(239,68,68,0.12)'};color:var(--t-ink)">
            <span class="h-1.5 w-1.5 rounded-full shrink-0" style="background:{statusColor(record.status)}"></span>
    ...
    <div class="border-t border-[var(--t-surface)] pt-2 flex flex-wrap gap-3 text-[0.875rem]">
      <div class="flex items-center gap-1.5">
        <span class="text-[var(--t-muted)]">Father:</span>
        <span class="text-[var(--t-ink-soft)] font-medium">{record.father_name || '—'}</span>
      </div>
      {#if record.gender}
        <div class="flex items-center gap-1.5">
          <span class="text-[var(--t-muted)]">Gender:</span>
          ...
          {#each record.education as edu, ei (ei)}
            <div class="rounded-lg border border-[var(--t-surface)] bg-[var(--t-surface-3)] p-2.5 space-y-1.5">
              ...
              <div class="grid grid-cols-1 gap-1.5 text-sm">
                ...
                <div class="flex flex-wrap gap-4"><span class="text-[var(--t-muted)]">HT No:</span> <span class="text-[var(--t-ink-soft)] font-medium tabular-nums">{edu['HT No'] || '—'}</span> {#if edu.From || edu.To}<span class="text-[var(--t-muted)]">From–To:</span> <span class="text-[var(--t-ink-soft)]">{edu.From || ''}{#if edu.From && edu.To} – {/if}{edu.To || ''}</span>{/if}</div>
              </div>
            </div>
          {/each}
        </div>
    ...
    {#if record.work_experience && (record.work_experience.Address || record.work_experience.State || record.work_experience.District || record.work_experience['Pin code'])}
      <div class="border-t border-[var(--t-surface)] pt-2 space-y-2">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[0.875rem]">
          <div>
            <dt class="text-[var(--t-muted)] text-[0.7rem] font-semibold uppercase tracking-wider">Address</dt>
          <dd class="text-[var(--t-ink-soft)] mt-0.5">{displayWork(record.work_experience.Address)}</dd>
        </div>
        <div>
          <dt class="text-[var(--t-muted)] text-[0.7rem] font-semibold uppercase tracking-wider">State</dt>
          <dd class="text-[var(--t-ink-soft)] mt-0.5">{displayWork(record.work_experience.State)}</dd>
        </div>
        <div>
          <dt class="text-[var(--t-muted)] text-[0.7rem] font-semibold uppercase tracking-wider">District</dt>
          <dd class="text-[var(--t-ink-soft)] mt-0.5">{displayWork(record.work_experience.District)}</dd>
        </div>
        <div>
          <dt class="text-[var(--t-muted)] text-[0.7rem] font-semibold uppercase tracking-wider">Pin Code</dt>
